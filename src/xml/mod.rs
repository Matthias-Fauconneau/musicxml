		...
		visitor.visit_map(::serde::de::value::MapDeserializer::new(std::iter::from_fn(|| {
			...
			while let Some(a) = attributes.next() {
				if let Some((field,_)) = fields.take_first(|(_,(name,_))| name == &a.name()) {
					...
					//eprintln!("attribute {:?}={}", field, a.value());
					...
					//eprintln!("no field '{}' in {:?}, deserializing attribute to '{}'", a.name(), fields, field);
					/*if !(def.starts_with("+") || def.starts_with("*") || def.starts_with("{"))*/ {
						fields.remove(index);
						//eprintln!("remove field with single occurence from possible choices {:?}", field);
					} /*else {
						eprintln!("keep field with multiple occurence in possible choices {:?}", field);
					}*/
					//index += 1;
					//("", Value::Text(TextDeserializer(a.value()))) // Flatten
					return Some((field, Value::Content(ContentDeserializer(node)))); // Flatten
				}/*else {
					eprintln!("Unexpected attribute {a:?} in {_name}, try parent");
					None
				}*/
			}// else {
			while let Some(child) = node.children.peek() {
				...
				if !name.is_empty() /*&&*/{
					...
					let (field,(_,_def)) = fields[index];
					//eprintln!("no field '{}' in {:?}, deserializing child to '{}'.'{}'", name, fields, node.name, field);
					/*if !(def.starts_with("+") || def.starts_with("*") || def.starts_with("{"))*/ {
						fields.remove(index);
						//eprintln!("remove field with single occurence from possible choices {:?}", field);
					} /*else {
					...
					//index += 1;
					//eprintln!("External enum tag {fields:?} {index} '{field}' {node:?}");
					...
					//eprintln!("deserializing text child to {}", field);
					//break Some((field, Value::Content(ContentDeserializer(node)))); // External enum tag
					//if !child.text().unwrap().trim().is_empty() { eprintln!("deserializing text {:?} to {}", child, field); }
...
impl<'de> Deserializer<'de> for &mut ElementDeserializer<'de> {
	...
	#[throws] fn deserialize_struct<V: Visitor<'de>>(self, name: &'static str, fields: &'static [&'static str], visitor: V) -> V::Value {
		...
		use itertools::Itertools; 
		//assert!(self.attributes.peek().is_none(), "Remaining attributes '{:?}' in {:?}", self.attributes.clone().format(" "), self);
	...
	#[allow(unreachable_code)] #[throws] fn deserialize_enum<V: Visitor<'de>>(self, name: &'static str, variants: &'static [&'static str], visitor: V) -> V::Value {
		if name ==  self.name { // e.g sign: clef-sign
			//eprintln!("content enum '{}' {:?} {:?}", name, variants, self.name);
			TextDeserializer(self.simple_content()?).deserialize_enum(name, variants, visitor)?
		} else { // e.g encoding
			//eprintln!("tag enum '{}' {:?} {:?}", name, variants, self.name);
    ...
    #[throws] fn deserialize_any<V: Visitor<'de>>(self, visitor: V) -> V::Value {
		//eprintln!("any {}", &visitor as &dyn de::Expected);
