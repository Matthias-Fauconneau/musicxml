...
impl<'de> de::Deserializer<'de> for EmptySeqDeserializer {
	...
	#[throws] fn deserialize_any<V: Visitor<'de>>(self, _visitor: V) -> V::Value { panic!(/*"empty seq any {}'", &visitor as &dyn de::Expected*/); }
...
pub(super) struct SeqDeserializer<'t, 'de>{pub children: std::cell::RefMut<'t, std::iter::Peekable<roxmltree::Children<'de, 'de>>>, pub tag: &'t str}
...
impl<'t, 'de> de::Deserializer<'de> for SeqDeserializer<'t, 'de> {
				...
				if let Some(child) = self.children.peek() {
					if child.is_element() {
						if child.tag_name().name() == self.tag {
							break Some(ElementDeserializer::new(self.children.next().unwrap())) // Leave content context
						} else {
							break None;
						}
					} else {
	...
	#[throws] fn deserialize_any<V: Visitor<'de>>(self, _visitor: V) -> V::Value {
		panic!(/*"Got seq {:?} {}, expected {}'", self.node, self.tag, &_visitor as &dyn de::Expected*/);
	}
	serde::forward_to_deserialize_any!{
		char bytes byte_buf str string identifier bool u8 u16 u32 u64 u128 i8 i16 i32 i64 i128 f32 f64 option unit map unit_struct newtype_struct tuple tuple_struct struct enum ignored_any}
}
